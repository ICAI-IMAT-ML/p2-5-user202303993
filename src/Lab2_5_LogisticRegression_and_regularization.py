...
class LogisticRegressor:
    ...
    def fit(
        ...
        X (np.ndarray): Matriz de características de tamaño (m, n), donde:

        m es el número de ejemplos.
        n es el número de características.
        y (np.ndarray): Vector de etiquetas de tamaño (m,).
        ...
        penalty (str o None): Tipo de regularización a aplicar:

        'lasso' (L1) → Favorece la eliminación de coeficientes innecesarios.
        'ridge' (L2) → Favorece pesos más pequeños en general.
        'elasticnet' → Combinación de L1 y L2 (controlada por l1_ratio).
        None → Sin regularización.
        l1_ratio (float): Parámetro de mezcla para ElasticNet:

        0 → Ridge puro.
        1 → Lasso puro.
        0.5 → Combinación de ambos.
        C (float): Inverso de la fuerza de regularización (1/λ):

        Valores bajos → Más regularización.
        Valores altos → Menos regularización.
        verbose (bool): Si True, imprime la pérdida cada print_every iteraciones.

        print_every (int): Frecuencia con la que se imprime la pérdida."""
        
        # TODO: Obtain m (number of examples) and n (number of features)
        m = X.shape[0] #devuelve el numero de filas 
        n = X.shape[1]

        # TODO: Initialize all parameters to 0
        self.weights =  np.array(np.zeros(n)) #inicializamos los parametros a 0
        self.bias = 0
        ...
        for i in range(num_iterations):  # Fill the None here
            ...
            X_bias = np.transpose(np.hstack((np.ones((X.shape[0], 1)), X))) #np.ones crea la columna de 1, hstack añade la columna a X, y finalmente trasponemos
            f = (1/m) * (X_bias)@(y_hat-y).astype("float")  #formula del gradiente
            dw = f[1:]   # Derivative w.r.t. the coefficients
            db = f[0]    # Derivative w.r.t. the intercept
    ...
    def predict_proba(self, X):
        ...
        z = X@self.weights+self.bias #z=Xw+b
    ...
    def lasso_regularization(self, dw, m, C):
        ...
        lasso_gradient = lasso_gradient = (C/m) * np.sign(self.weights)
    ...
    def ridge_regularization(self, dw, m, C):
        ...
        ridge_gradient = (C/m)*self.weights #como le añadimos la derivada , no hace falta ponerlo al cuadrado
    ...
    def elasticnet_regularization(self, dw, m, C, l1_ratio):
        ...
        lasso = self.lasso_regularization(dw, m, C) - dw #como devuleve lasso_regularization dw + regularization , le restamos dw para quedarno solos con la regularizacion
        ridge = self.ridge_regularization(dw, m, C) - dw
        elasticnet_gradient = l1_ratio * lasso + (1 - l1_ratio) * ridge #multiplicamos por el ratio y por el complementario (1-ratio) a cada uno 
